...
- 智能能力：静态追溯基线、可选 OpenAI-compatible LLM、Agent 工具白名单与人工确认
...
5. 查看真实静态分析生成的张量流图，点击节点跳转到对应代码。
6. 生成追溯候选并人工接受或拒绝；未配置 LLM 时自动降级为静态结果。
7. 在 Agent 页签提问；保存代码、重跑分析或更新追溯状态必须先确认。
8. 通过右上角设置窗口切换 Agent 服务或 MinerU 本地/官方接入，无需重启。
9. 返回项目入口，点击“批量管理”，可全选或勾选多个项目并永久删除其关联数据。
