...
from tracelab_server.auth.dependencies import CurrentIdentity, require_platform_admin
from tracelab_server.auth.service import audit, user_read
...
from tracelab_server.schemas.cloud import AdminQuotaPatch, AdminUserPatch, UserRead
...
def list_project_metadata(
    ...
    """Project metadata only; no domain payload or Blob download handle is returned."""
    statement = select(CloudProject)
    if workspace_id:
        statement = statement.where(CloudProject.workspace_id == workspace_id)
    rows = session.exec(statement.order_by(CloudProject.updated_at.desc())).all()
    return [
        {
