  ...
  >
    ...
    <div v-else class="evidence-content">
      ...
      <section class="evidence-actions">
        <el-button type="primary" :disabled="!row.id" @click="$emit('confirm', row)">
          确认追溯关系
        </el-button>
        <el-button :disabled="!row.id" @click="$emit('reject', row)">
          驳回
        </el-button>
      </section>
      <p v-if="!row.id" class="review-hint">当前为工作台预览数据，生成候选后可审阅。</p>
    </div>
  </el-drawer>
</template>
