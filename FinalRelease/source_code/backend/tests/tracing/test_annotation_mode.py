...
from app.services.tracing.manual_anchors import code_span_quote, paper_block_text
...
def test_anchor_helpers_cover_flat_blocks_symbols_files_and_unreadable_content(
    monkeypatch,
) -> None:
    paper = PaperDocument(
        project_id=1,
        ...
        storage_path="paper.pdf",
        pages_json=[],
        paragraphs_json=[{"id": "flat-p1", "text": "  Flat paragraph evidence.  "}],
    )
    assert paper_block_text(paper, "flat-p1") == "Flat paragraph evidence."

    code = CodeRepository(
        project_id=1,
        filename="repo.zip",
        storage_path="repo.zip",
        file_tree_json=[{"path": "models/net.py"}],
        symbols_json=[
            {"id": "other", "path": "other.py", "line_start": 1, "line_end": 1},
            {
    ...
    payloads = {
        "models/net.py": {"content": "header\nclass Net:\n    pass\n"},
        "empty.py": {"content": ""},
    }
    code.file_tree_json.append({"path": "empty.py"})
    monkeypatch.setattr(
        "app.services.workspace_service.get_code_file",
        lambda _session, _project_id, path: payloads.get(path),
    )

    with _session_for_helpers() as session:
        assert code_span_quote(session, 1, code, "models/net.py::Net") == (
            "class Net:",
            "models/net.py",
            2,
            2,
        )
        assert code_span_quote(session, 1, code, "models/net.py") == (
            "header",
            "models/net.py",
            1,
            1,
        )
        assert code_span_quote(session, 1, code, "missing.py:4-8") == (
            "",
            "missing.py",
            4,
            8,
        )
        assert code_span_quote(session, 1, code, "empty.py") == (
            "",
            "empty.py",
            1,
            1,
...
def _session_for_helpers() -> Session:
    engine = create_engine(
        "sqlite://",
        connect_args={"check_same_thread": False},
        poolclass=StaticPool,
