...
class AdminWebSession(CloudSQLModel, table=True):
    """Opaque, cookie-bound session used only by the server-owned admin console."""

    __tablename__ = "admin_web_session"
    session_id: str = Field(default_factory=new_uuid, primary_key=True, max_length=36)
    user_id: str = Field(foreign_key="user_account.user_id", index=True, max_length=36)
    token_hash: str = Field(unique=True, index=True, max_length=64, repr=False)
    csrf_hash: str = Field(max_length=64, repr=False)
    expires_at: datetime = Field(index=True)
    created_at: datetime = Field(default_factory=utc_now)
