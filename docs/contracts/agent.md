...
**深度思考（`project.agent_deep_thinking`，项目级，默认关闭）**决定 `confidence` 的产生方式。关闭时即上述默认行为：Agent 直接给出 `confidence`，原样落库，prompt 也只要求三个分数。开启时父 Agent 与区域子代理改为逐候选给出六个维度 `change_directness`(0.20)/`causal_reachability`(0.25)/`requirement_support`(0.20)/`trace_support`(0.15)/`verification_support`(0.10)/`context_coverage`(0.10)（各 `[0,1]`）与可选减分项 `confidence_penalties`（`paper_association_inferred` -0.10、`no_call_entry` -0.20、`alternate_implementation` -0.20、`config_or_caller_unread` -0.15、`context_truncated` -0.15、`runtime_condition_unverified` -0.15），由 `analysis_tools.compute_trace_confidence` 在服务端按加权和加减分项算出 `confidence`（截断到 `[0,1]`，保留 4 位），未知减分键忽略；六维全为默认值且无减分项时回退到模型自报的 `confidence`。此时 `score_basis_json` 额外记录 `confidence_dimensions` 与 `confidence_penalties`，`prompt_version` 为 `trace-agent-v3`（关闭时为 `trace-agent-v2`）。开关经 `PATCH /projects/{id}` 的 `agent_deep_thinking` 读写，只影响之后启动的分析任务；多维评估会明显拉长每批发布耗时，因此默认关闭。
