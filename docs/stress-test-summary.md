...
**写入吞吐稳定在 26–35 rps**，从 N=10 到 N=240 保持平台期 —— 说明系统在过载时是平滑退化，而非崩塌。内存全程宽裕（248 MiB / 3 GiB），瓶颈是 CPU 预算，扩容路径明确。

**尖峰自愈能力（S10）**：10→200 并发在 10 秒内拉起、保持 2 分钟，随后回落。
...
失败在 **60 秒内归零**，吞吐与延迟在 **75 秒内完全恢复**到尖峰前水平，压测结束后健康检查正常，**全程无需人工干预**。
