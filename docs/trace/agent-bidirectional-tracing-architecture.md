...
| 颜色 | 含义 |
| --- | --- |
| 蓝（下划线/浅底） | `proposed`：Agent 生成、尚未审阅 |
| 绿（下划线/浅底） | `accepted`：已人工确认 |
| 紫（下划线，角标 `×N`） | 一对多：同一论文目标对应 N 处代码 |
| 青（下划线，角标 `×N`） | 多对一：N 处论文目标对应同一处代码 |
| 紫+青双下划线 | 多对多：同时存在扇出与扇入 |
| 黄（实底 + 描边） | 当前**选中**的那一条关系，两侧同时标出 |
| 浅黄 | 鼠标**悬停预览**，不移动视图、不改变选中 |

蓝/绿表示审阅状态，紫/青表示关系基数，黄表示当前焦点——三者相互独立、可叠加。角标 `×N` 中的 N 是对侧目标数量。

#### 公式与块级目标的锚定

公式、表格、图片属于块级目标：无法在其中包裹 `<mark>`，因此整块加 `trace-block-target`。这类目标有两个必须处理的现实问题：

1. **quote 与 DOM 文本对不上。** Agent 引用的是 markdown 源码里的 TeX（`$$\hat{y}=Wx+b$$`），而 DOM 里是 KaTeX 渲染出的字形，文本比对必然失败。定位改为比较 KaTeX 自带的 `<annotation encoding="application/x-tex">` 与 quote 的**归一化 TeX**（`texKey()`：剥离 `$`、`\left`/`\right`、`\,` 等间距宏与空白花括号）。找不到匹配时目标记为 `unresolved`，**绝不回退到“文档里第一个公式”**——高亮到无关公式看起来就是一条错误的追溯结果，比明确报告定位失败更糟。
2. **一个块可能承载多条关系。** 同一个公式被两处代码引用，或同一段落含两个片段时，块上必须保留全部 target id（`data-trace-targets`，逗号分隔），`data-trace-target` 仅作为主 id 供既有 `closest()` 查询使用。只保留第一个 id 会让其余关系既失去常驻高亮、又把 hover/点击错误地指向另一条关系——这正是“公式只在选中时临时高亮几秒、平时没有常驻标记”的成因。

常驻装饰与选中/悬停态是两层：`decoratePaperTargets` 负责常驻标记，`setActivePaperTargets` 只切换 `trace-target-active`/`trace-target-hover`，两者都按 target id 集合判断，因此一个块上的多条关系互不覆盖。
...
**重新生成前必须询问是否保留历史。** 追溯是一个逐步收敛的过程，用户可能已经接受/拒绝了一部分关系，静默清空会丢掉这些判断。工作台在点击“重新生成”时弹窗二选一：

- **保留历史记录**（默认）：直接重跑。重跑按 fingerprint upsert，`accepted`/`rejected` 永不被覆盖，新旧结果合并。
- **清空后重新生成**：先调用 `DELETE /trace-links?scope=proposed` 删除未审阅候选，再重跑，矩阵只显示本轮结果。

中途“中止（保留已发现）”产出的关系与正常产出完全等价——它们已经落库，下一轮重跑会在其基础上继续 upsert，不需要额外处理。

**调试模式**（默认关闭，`localStorage: tracelab.debug.enabled`）：开启后，界面捕获的每个错误都带完整信息记入调试面板（HTTP 状态、服务端 `detail`、请求方法与 URL、堆栈），并在分析任务失败时自动拉取 `/analysis-jobs/{job_id}/diagnostics`，展示 provider/模型、`degraded_reason`、失败的工具调用与模型步骤轨迹。面板可复制导出。关闭时不产生任何额外请求。
